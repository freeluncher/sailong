                ...
                @foreach ($accommodations as $accommodation)
                    <div class="bg-white rounded-lg shadow-lg overflow-hidden w-full h-auto sm:w-60 md:w-64">
                        <div class="w-full h-48 overflow-hidden">
                            <img class="object-cover w-full h-full" src="{{ Storage::url('img/' . $accommodation->image) }}"
                                alt="{{ $accommodation->name }}">
                        </div>
                        <div class="p-6">
                            <h2 class="text-xl font-bold mb-2 truncate">{{ $accommodation->name }}</h2>
                            <p class="text-gray-700 mb-4 truncate">{{ $accommodation->location }}</p>
