                ...
                @foreach ($cuisines as $cuisine)
                    <div class="bg-white rounded-lg shadow-lg overflow-hidden w-full h-auto sm:w-60 md:w-64">
                        <div class="w-full h-64 md:h-64 overflow-hidden">
                            <img class="object-cover w-full h-full" src="{{ asset('img/' . $cuisine->image) }}"
                                alt="{{ $cuisine->name }}">
                        </div>
                        <div class="p-6">
                            <h2 class="text-xl font-bold mb-2">{{ $cuisine->name }}</h2>
                            <p class="text-gray-700 mb-4">{{ $cuisine->location }}</p>
                            <p class="text-gray-600">{{ $cuisine->description }}</p>
                            </p>
