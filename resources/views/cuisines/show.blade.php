...
@section('content')
    <div class="container mx-auto p-4 max-w-5xl">
        <!-- Header Back Button -->
        <div class="mb-4">
            <a href="{{ route('cuisines.index') }}" class="inline-flex items-center text-blue-700 hover:text-yellow-400 font-semibold">
        ...
        <div class="bg-white rounded-2xl shadow-xl p-6 md:p-10">
            ...
            <div class="mt-8">
                <h1 class="text-3xl font-bold text-blue-900 mb-2">{{ $cuisine->name }}</h1>
                <p class="mt-2 text-lg text-blue-700">{{ $cuisine->description }}</p>
            </div>
            ...
            <div class="mt-8 flex flex-col md:flex-row justify-between items-center gap-6">
                ...
                <div class="flex flex-col space-y-2 text-center md:text-left">
                    <div class="flex items-center">
                        ...
                        <span class="text-blue-900 font-semibold">{{ $cuisine->location }}</span>
                    </div>
                    <div class="flex items-center">
                        <i class="fas fa-clock text-yellow-400 mr-2"></i>
                        <span class="text-blue-900">Buka {{ $cuisine->opening_hours ?? '-' }} - {{ $cuisine->closing_hours ?? '-' }}</span>
                    </div>
                    <div class="flex items-center">
                        <i class="fas fa-ticket-alt text-yellow-400 mr-2"></i>
                        <span class="text-blue-900 font-bold text-lg">Mulai dari Rp{{ number_format($cuisine->ticket_price, 0, ',', '.') }} <span class="text-xs font-normal">/ menu</span></span>
                    </div>
                </div>

                <!-- Action Buttons -->
                <div class="flex flex-wrap gap-2 mt-4 md:mt-0">
                    @if (!empty($cuisine->action_buttons))
                        @foreach ($cuisine->action_buttons as $button)
                            <a href="{{ $button['url'] }}"
