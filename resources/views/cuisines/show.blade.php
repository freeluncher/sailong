...
@section('content')
    <div class="relative h-screen w-full overflow-hidden">
        ...
        <div class="absolute left-12 top-24 inset-0 flex flex-col items-start justify-start">
            <h1 class="text-4xl md:text-5xl lg:text-6xl text-white font-bold">{{ $cuisine->name }}</h1>
            <p class="text-lg md:text-xl lg:text-2xl text-white mt-4">{{ $cuisine->location }}</p>
            <p class="text-lg md:text-xl lg:text-2xl text-white mt-4">{{ $cuisine->description }}</p>
            </p>
        </div>
    </div>
    <div class="mt-8 mx-auto px-10">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            @foreach ($cuisine->gallery as $gallery)
                <div>
